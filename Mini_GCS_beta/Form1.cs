...
namespace Mini_GCS_beta
{
    public partial class Form1 : Form
    {
         ...
         *  Private variables
         */
        private ConcurrentQueue<byte[]> queue_serial_ch1_raw = new ConcurrentQueue<byte[]>();
        private subscription_manager DFrame_sub_manager = new subscription_manager();
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            //gmap.Position = new PointLatLng(22.4160124862853, 114.04306948185);
            gmap.Position = new PointLatLng(23.10611, 120.21083);
            this.gmap.MouseMove += new MouseEventHandler(gmap_MouseMove);
            this.gmap.MouseClick += new MouseEventHandler(gmap_MouseClick);
            String[] tmp;
            int i;
        ...
        /**
         *  Try to find and show available ports
         */
        ...
        /**
         *  Connect to specified serial port, using user specified baud rate
         */
        private void button1_Click(object sender, EventArgs e)
        {
            if (button1.Text == "Connect")
            {
                serial_ch1.PortName = comboBox1.SelectedItem.ToString();
                serial_ch1.BaudRate = int.Parse(textBox1.Text);
                serial_ch1.Parity = Parity.None;
                serial_ch1.DataBits = 8;
                serial_ch1.StopBits = StopBits.One;
                serial_ch1.Handshake = Handshake.None;

                serial_ch1.ReadTimeout = 500;
                serial_ch1.WriteTimeout = 500;

                serial_ch1.Open();
                serial_ch1.DiscardInBuffer();
            ...
            else if (button1.Text == "Disconnect")
            {
                ...
                backgroundWorker_serial_ch1.CancelAsync();
            }
        }

        /**
         * serial_ch1 data received handler
         */
        private void serial_ch1_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            lock (sp)
            {
                ...
                sp.Read(buf, 0, cnt);
                // fifo buffer the bytes (thread safe)
                queue_serial_ch1_raw.Enqueue(buf);
            }
            

            // trigger background worker to process the bytes
            if (backgroundWorker_serial_ch1.IsBusy == false)
            {
                backgroundWorker_serial_ch1.RunWorkerAsync();
            }
        }
...
/*--------------------------------------------------------------------*/
 ...
 0xB40BBE37, 0xC30C8EA1, 0x5A05DF1B, 0x2D02EF8D
        ...
        /**
         *  calculate CRC32 value using lookup table
         */
        private UInt32 crc32_byte(byte[] p, int bytelength)
        {
